...
export class ProcessesApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async executeImpactMetrics(impactMetricsProcessParams: ImpactMetricsProcessParams, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'impactMetricsProcessParams' is not null or undefined
        if (impactMetricsProcessParams === null || impactMetricsProcessParams === undefined) {
            throw new RequiredError("ProcessesApi", "executeImpactMetrics", "impactMetricsProcessParams");
        }
        ...
        requestContext.setHeaderParam("Content-Type", contentType);
        const serializedBody = ObjectSerializer.stringify(
            ObjectSerializer.serialize(impactMetricsProcessParams, "ImpactMetricsProcessParams", ""),
            contentType
        );
        requestContext.setBody(serializedBody);
...
export class ProcessesApiResponseProcessor {
     ...
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async executeImpactMetricsWithHttpInfo(response: ResponseContext): Promise<HttpInfo<BiodiversitySensitiveAreasProcessOutputs >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
